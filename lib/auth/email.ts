...
// Email template untuk OTP Login
export function getLoginOTPEmailHtml(otpCode: string, name?: string): string {
  return `<!DOCTYPE html>
<html>
<head>
  <meta charset="utf-8">
  <title>Verifikasi Login - Triablazer</title>
  <style>
    body { font-family: 'Rajdhani', sans-serif; background: #050810; color: #E5DCC8; margin: 0; padding: 20px; }
    .container { max-width: 500px; margin: 0 auto; background: #0B1121; border: 0.5px solid rgba(200,169,110,0.18); padding: 2rem; border-radius: 12px; }
    .code { font-size: 2rem; font-weight: 700; letter-spacing: 8px; color: #C8A96E; text-align: center; padding: 1rem; background: rgba(200,169,110,0.05); margin: 1.5rem 0; }
...
// Fungsi utama send email dengan error handling
export async function sendOTPEmail(email: string, code: string, name?: string, type: 'verification' | 'login' = 'verification') {
  try {
    const html = type === 'login' 
      ? getLoginOTPEmailHtml(code, name)
      : getVerificationEmailHtml(code, name);
    
    const subject = type === 'login' 
      ? 'Kode Verifikasi Login - Triablazer'
      : 'Kode Verifikasi - Triablazer';
...
// Hapus generateOTP dari sini karena sudah ada di route utama
// Jangan definisikan generateOTP() di sini untuk menghindari duplikasi